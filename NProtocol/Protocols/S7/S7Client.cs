...
namespace NProtocol.Protocols.S7
{
    public partial class S7Client : DriverBase
    {
        ...
        public S7Client(EtherNetParameter parameter, TsapPair tsapPair) : this(parameter)
        {
            TsapPair = tsapPair;
        }
        ...
        {
            if (receiveData.Length < 4) return false;
            if (receiveData[0] != sendData[0] && receiveData[1] != sendData[1]) return false;
            if (receiveData.Length != receiveData[2] * 256 + receiveData[3]) return false;
                            ...
                            case S7CommPduType.Ack:
                                {
                                        ...
                                        receiveData[7] == ProtocolId)
                                    {
                                        if (receiveData[17] != 0 || receiveData[18] != 0)
                                        {
                                            ...
                                            throw new ReceivedException($"The header message is incorrect，S7ErrorClass：{s7ErrorClass},S7ErrorCode：{s7ErrorCode}", sendData, receiveData, DriverId);
                                        }
