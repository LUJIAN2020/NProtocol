...
namespace NProtocol.Protocols.S7.Extensions
{
    public static class S7StringExtension
    {
        ...
        {
            if (bytes.Length < 2)
                throw new ArgumentOutOfRangeException(nameof(bytes.Length), bytes.Length, "The array length must be > 2");
            if (bytes[1] > bytes[0])
                throw new ArgumentOutOfRangeException(nameof(bytes), bytes[1], "Maximum character length must be ≥ actual character length");
        ...
        {
            if (bytes.Length < 4)
                throw new ArgumentOutOfRangeException(nameof(bytes), bytes.Length, "The array length must be > 4");
            int size = (bytes[0] << 8) | bytes[1];
            int length = (bytes[2] << 8) | bytes[3];
            if (length > size)
                throw new ArgumentOutOfRangeException(nameof(bytes), length, "Maximum character length must be ≥ actual character length");
        ...
        public static byte[] ToBytesFromS7String(this string value, int reservedLength)
        {
            if (reservedLength > S7StringMaximumLength)
                throw new ArgumentOutOfRangeException(nameof(reservedLength), reservedLength, $"Maximum character length <= {S7StringMaximumLength}");
            var bytes = Encoding.ASCII.GetBytes(value);
            if (bytes.Length > reservedLength)
                throw new ArgumentOutOfRangeException(nameof(value), value.Length, "The actual content length is greater than the specified length");
        ...
        public static byte[] ToBytesFromS7WString(this string value, int reservedLength)
        {
            if (reservedLength > S7WStringMaximumLength)
                throw new ArgumentOutOfRangeException(nameof(reservedLength), reservedLength, $"Maximum character length <= {S7WStringMaximumLength}");
