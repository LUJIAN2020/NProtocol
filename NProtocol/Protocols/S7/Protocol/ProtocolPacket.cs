...
namespace NProtocol.Protocols.S7
{
    ...
    public partial class S7Client
    {
        ...
        /// </summary>
        /// <returns></returns>
        private byte[] CreateS7CommPacket(S7CommPduType s7CommPduType,
            ...
            byte[]? writeData = default)
        {
            ...
            var parameter = CreateS7CommParameterPacket(s7CommFuncCode, areaType, varType, db, byteAddress, bitAddress, count);
        ...
        /// <returns></returns>
        private byte[] CreateS7CommParameterPacket(S7CommFuncCode s7CommFuncCode,
            S7MemoryAreaType areaType = S7MemoryAreaType.DataBlock,
            S7VarType varType = S7VarType.Bit,
            ushort db = 1,
            int byteAddress = 0,
                ...
                case S7CommFuncCode.WriteVar:
                    {
                        ...
                        buffer[0] = (byte)s7CommFuncCode;//作业请求（Job）和确认数据响应（Ack_Data）
                        buffer[1] = 1;
                        var item = CreateParameterItemPacket(varType, areaType, db, byteAddress, bitAddress, count);
        ...
        /// <param name="count"></param>
        /// <returns></returns>
        private byte[] CreateParameterItemPacket(S7VarType varType, S7MemoryAreaType areaType, ushort db, int byteAddress, byte bitAddress, ushort count)
        {
        ...
        {
            if (writeData is null) return Array.Empty<byte>();
