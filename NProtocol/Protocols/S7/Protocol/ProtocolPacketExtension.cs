...
namespace NProtocol.Protocols.S7
{
    ...
    public partial class S7Client
    {
        ...
        )
        {
            ...
            byte[] len = (
                (ushort)(
                    tpkt.Length + cotp.Length + header.Length + parameter.Length + payload.Length
                )
            ).ToBytes();
            tpkt[2] = len[1];
            tpkt[3] = len[0];
        ...
        public Result<DateTime> GetCpuDateTime()
        {
            return EnqueueExecute(() =>
            {
                ...
                var sendData = CreateExtensionPacket(
                    UserDataFunction.TimeFunctions,
                    (byte)TimeSubFunction.ReadClock,
                    data
                );
                ...
                int millisecond = Convert.ToInt32(
                    (readBytes[8] * 256 + (readBytes[9] & 0xF0)).ToString("x2")
                );
                string timeString =
        ...
        public Result<CpuStatus> ReadCpuStatus()
        {
            return EnqueueExecute(() =>
            {
                ...
                var sendData = CreateExtensionPacket(
                    UserDataFunction.CPUFunctions,
                    (byte)CpuSubFunction.ReadSZL,
                    data
                );
                var result = NoLockExecute(sendData);
                var receivedData = result.ReceivedData;
                ...
                int millisecond = Convert.ToInt32(
                    (dateTimeBuffer[6] * 256 + (dateTimeBuffer[7] & 0xF0)).ToString("x2")
                );
                string timeString =
                    $"{year:00}-{month:00}-{day:00} {hour:00}:{minute:00}:{second:00}.{millisecond:000}";
                DateTime.TryParse(timeString, out DateTime dateTime);
        ...
        public Result SetCpuColdRestart()
        {
            return EnqueueExecute(() =>
            {
                ...
                var piServicePayload = new byte[]
                {
                    ...
                    0xFD, //unknown
                    0x00,
                    0x02, //parameter block length
                    0x43,
                    0x20, //parameter block "C "
                    0x09, //string length
                    0x50,
                    0x5F,
                    0X50,
