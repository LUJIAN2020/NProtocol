...
namespace NProtocol.Protocols.S7.StructType
{
    ...
    {
        private static TValue GetValueOrThrow<TValue>(FieldInfo fi, object structValue)
            where TValue : struct
        {
            if (fi.GetValue(structValue) is TValue value)
                return value;
            throw new ArgumentException(
                $"Failed to convert value of field {fi.Name} of {structValue} to type {typeof(TValue)}"
            );
        }
        ...
        public static int GetStructSize(Type structType)
        {
            ...
            foreach (var info in infos)
            {
                switch (info.FieldType.Name)
                {
                    ...
                    case nameof(String):
                    {
                        var attribute = info.GetCustomAttributes<S7StringAttribute>()
                            .SingleOrDefault();
                        if (attribute == default(S7StringAttribute))
                            throw new ArgumentException(
                                "Please add S7StringAttribute to the field"
                            );
                    ...
                    {
                        var attribute = info.GetCustomAttributes<S7TimeAttribute>()
                            .SingleOrDefault();
                    ...
                    {
                        var attribute = info.GetCustomAttributes<S7DateTimeAttribute>()
                            .SingleOrDefault();
                        if (attribute == default(S7DateTimeAttribute))
                            throw new ArgumentException(
                                "Please add S7DateTimeAttribute to the field"
                            );
                    ...
                    default:
                        if (
                            info.FieldType.Name == nameof(Int64)
                            || info.FieldType.Name == nameof(UInt64)
        ...
        public static object ToStruct(Type type, byte[] bytes)
        {
            ...
            if (bytes.Length != GetStructSize(type))
                throw new ArgumentOutOfRangeException(
                    nameof(bytes.Length),
                    bytes.Length,
                    "The array length does not match the struct length."
                );
            double byteCount = 0.0;
            ...
            foreach (var info in infos)
            {
                switch (info.FieldType.Name)
                {
                    ...
                    {
                        var attribute =
                            info.GetCustomAttributes<S7StringAttribute>().FirstOrDefault()
                            ?? throw new ArgumentException(
                                "Please add S7StringAttribute to the field"
                            );
                        ...
                        switch (attribute.Type)
                        {
                            ...
                            default:
                                throw new ArgumentException(
                                    "Please use a valid string type for the S7StringAttribute"
                                );
                        }
                    ...
                    case nameof(TimeSpan): //TimeSpan
                    {
                        ...
                        if (byteCount / 2 - Math.Floor(byteCount / 2.0) > 0)
                            byteCount++;
                        var attribute =
                            info.GetCustomAttributes<S7TimeAttribute>().FirstOrDefault()
                            ?? throw new ArgumentException(
                                "Please add S7TimeAttribute to the field"
                            );
                        ...
                        switch (attribute.Type)
                        {
                            ...
                            default:
                                throw new ArgumentException(
                                    "Please use a valid type for the S7TimeAttribute"
                                );
                        }
                    ...
                    case nameof(DateTime): //DateTime
                    {
                        ...
                        var attribute =
                            info.GetCustomAttributes<S7DateTimeAttribute>().FirstOrDefault()
                            ?? throw new ArgumentException(
                                "Please add S7DateTimeAttribute to the field"
                            );
                        ...
                        dateTime = attribute.Type switch
                        {
                            ...
                            _ => throw new ArgumentException(
                                "Please use a valid type for the S7DateTimeAttribute"
                            ),
                        };
        ...
        public static byte[] ToBytes(object structValue)
        {
            ...
            foreach (var info in infos)
            {
                ...
                switch (info.FieldType.Name)
                {
                    ...
                    {
                        var attribute =
                            info.GetCustomAttributes<S7StringAttribute>().SingleOrDefault()
                            ?? throw new ArgumentException(
                                "Please add S7StringAttribute to the field"
                            );

                        var obj =
                            info.GetValue(structValue)
                            ?? throw new ArgumentNullException(nameof(structValue));

                        string content = (string)obj;
                        bytes2 = attribute.Type switch
                        {
                            S7StringType.S7String => content.ToBytesFromS7String(
                                attribute.ReservedLength
                            ),
                            S7StringType.S7WString => content.ToBytesFromS7WString(
                                attribute.ReservedLength
                            ),
                            _ => throw new ArgumentException(
                                "Please use a valid type for the S7StringAttribute"
                            ),
                        };
                        break;
                    }
                    ...
                    {
                        var attribute =
                            info.GetCustomAttributes<S7TimeAttribute>().SingleOrDefault()
                            ?? throw new ArgumentException(
                                "Please add S7TimeAttribute to the field"
                            );
                    ...
                    {
                        var attribute =
                            info.GetCustomAttributes<S7DateTimeAttribute>().SingleOrDefault()
                            ?? throw new ArgumentException(
                                "Please add S7DateTimeAttribute to the field"
                            );
