...
namespace NProtocol.Protocols.Modbus
{
    ...
    {
        public ModbusClient(SerialPortParameter parameter)
            : base(parameter, ConnectMode.SerialPort)
        {
        ...
        public ModbusClient(EtherNetParameter parameter, ModbusConnectMode mode)
            : base(parameter, GetConnectMode(mode))
        {
            ModbusConnectMode = mode;
        }

        public ModbusClient(string ip, ushort port, ModbusConnectMode mode)
            : this(EtherNetParameter.Create(ip, port), mode) { }

        private static ConnectMode GetConnectMode(ModbusConnectMode mode) =>
            mode switch
            {
