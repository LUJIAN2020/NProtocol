...
namespace NProtocol.Protocols.Mc
{
    ...
    public class Mc1EClient : McProtocolBase
    {
        ...
        public Mc1EClient(EtherNetParameter parameter, ConnectMode mode) : base(parameter, mode)
        {
            if (mode == ConnectMode.SerialPort)
                throw new ArgumentException("Serial port connection is not supported", nameof(mode));
        }
        public Mc1EClient(string ip, ushort port, ConnectMode mode) : this(EtherNetParameter.Create(ip, port), mode)
        {
        }
        private byte[] CreateCommand(Command1E cmd, PlcMemory1E plcMemory1E, uint address, ushort count, byte[]? writeData)
        {
            byte[] buffer = writeData is not null
                ? new byte[12 + writeData.Length]
                : new byte[12];
            //固定头部
        ...
        private void ValidateDataType<T>(T t, uint start, PlcMemory1E memory) where T : struct
        {
            switch (memory)
            {
                case PlcMemory1E.X:
                ...
                case bool:
                    {
                        ...
                        var bs = result.Payload.ToBooleansFromWord(IsLittleEndian).Slice((int)(address.StartAddress - start), count);
                        if (bs is T[] val)
        ...
        private Result WriteBytes(Command1E cmd, PlcMemory1E plcMemory1E, uint address, ushort count, byte[] writeData)
        {
            return EnqueueExecute(() =>
            {
        ...
        {
            var addr = new McAddress1E(address);
            return WriteBytes(Command1E.BatchWriteWord, addr.Memory, addr.StartAddress, (ushort)writeData.Length, writeData);
        }
        public Result WriteBytes(McAddress1E address, byte[] writeData)
        {
            return WriteBytes(Command1E.BatchWriteWord, address.Memory, address.StartAddress, (ushort)writeData.Length, writeData);
        }
        ...
        public Result WriteValues<T>(McAddress1E address, T[] writeData) where T : struct
        {
            T t = default;
            ValidateDataType(t, address.StartAddress, address.Memory);
                ...
                case bool[] bs:
                    {
                        ...
                        if (address.Memory == PlcMemory1E.D || address.Memory == PlcMemory1E.R)
                            throw new Exception($"The `{address.Memory}` address does not support write bits");
                        ...
                        return WriteBytes(Command1E.BatchWriteBit, address.Memory, address.StartAddress, (byte)count, bytes);
                    }
                case short[] values:
                    {
                        int count = writeData.Length;
        ...
        public Result WriteValue<T>(McAddress1E address, T value) where T : struct
        {
            return WriteValues(address, new T[] { value });
        }
        ...
        private bool ValidateReceivedData(byte[] sendData, byte[] receivedData)
        {
            ...
            else if (receivedData.Length == receivedData.Length &&
                receivedData[0] == sendData[0] + 0x80 &&
                receivedData[1] == OkEndCode)
            {
                //正确数据返回
