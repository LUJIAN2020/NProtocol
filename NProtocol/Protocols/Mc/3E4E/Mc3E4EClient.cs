...
namespace NProtocol.Protocols.Mc
{
    public class Mc3E4EClient : McProtocolBase
    {
        ...
        public Mc3E4EClient(EtherNetParameter parameter, ConnectMode mode) : base(parameter, mode)
        {
            if (mode == ConnectMode.SerialPort)
                throw new ArgumentException("Serial port connection is not supported", nameof(mode));
        }
        public Mc3E4EClient(string ip, ushort port, ConnectMode mode) : this(EtherNetParameter.Create(ip, port), mode)
        {
        }
        /// <summary>
        /// 报文帧格式
        ...
        private byte[] CreateCommand(Command3E4E main, SubCommand3E4E sub, PlcMemory3E4E memory, uint startAddress, byte count, byte[]? writeData = default)
        {
            ...
            switch (McFrame)
            {
                case McFrame.MC3E:
                    {
                        buffer = writeData is not null
                            ? new byte[21 + writeData.Length]
                            : new byte[21];
        ...
        private void ValidateDataType<T>(T t, uint start, PlcMemory3E4E memory) where T : struct
        {
            switch (memory)
            {
                ...
                case PlcMemory3E4E.CC:
                    switch (t)
                    {
                        ...
                        case uint:
                            ...
                            break;
                        default:
                            throw new ArgumentException("Data type not supported", t.GetType().ToString());
                    }
                    break;
        ...
        private int ReceivedDataSliceLength() => McFrame switch
        {
            McFrame.MC3E => 11,
            McFrame.MC4E => 15,
            _ => 0,
        };
        private Result ReadBytes(Command3E4E main, SubCommand3E4E sub, PlcMemory3E4E memory, uint startAddress, byte count)
        {
        ...
        {
            return ReadBytes(Command3E4E.BatchRead, SubCommand3E4E.Word, address.Memory, address.StartAddress, count);
        }
        public Result<T[]> ReadValues<T>(string address, byte count) where T : struct
        {
            var addr = new McAddress3E4E(address);
            return ReadValues<T>(addr, count);
        }
        public Result<T[]> ReadValues<T>(McAddress3E4E address, byte count) where T : struct
        {
            T t = default;
        ...
        public Result<T> ReadValue<T>(McAddress3E4E address) where T : struct
        {
            var result = ReadValues<T>(address, 1);
            var value = result.Value.FirstOrDefault();
            return result.ToResult(value);
        }
        private Result WriteBytes(Command3E4E main, SubCommand3E4E sub, PlcMemory3E4E memory, uint startAddress, byte count, byte[] writeData)
        {
            return EnqueueExecute(() =>
            {
        ...
        {
            return WriteBytes(Command3E4E.BatchWrite, SubCommand3E4E.Word, address.Memory, address.StartAddress, (byte)writeData.Length, writeData);
        }
        public Result WriteValues<T>(string address, T[] value) where T : struct
        {
            var addr = new McAddress3E4E(address);
            return WriteValues(addr, value);
        }
        public Result WriteValues<T>(McAddress3E4E address, T[] writeData) where T : struct
        {
            T t = default;
            ValidateDataType(t, address.StartAddress, address.Memory);
                ...
                case bool[] bs:
                    {
                        if (writeData.Length % 2 > 0)
                            throw new ArgumentOutOfRangeException(nameof(writeData.Length), writeData.Length, "Write length is a multiple of 2");
                        ...
                        return WriteBytes(Command3E4E.BatchWrite, SubCommand3E4E.Bit, address.Memory, address.StartAddress, (byte)count, bytes);
                    }
                case short[] values:
                    {
                        int count = writeData.Length;
        ...
        public Result WriteValue<T>(string address, T value) where T : struct
        {
            return WriteValues(address, new T[] { value });
        }
        public Result WriteValue<T>(McAddress3E4E address, T value) where T : struct
        {
            return WriteValues(address, new T[] { value });
        }
        ...
        {
            if (receivedData.Length < 11) return false;
            switch (McFrame)
            {
                case McFrame.MC3E:
                    {
                        ...
                        if (sendData[0] + 0x80 == receivedData[0] &&
                        ...
                        else
                        {
                            ...
                            throw new ReceivedException($"Response data error, error code:{errorCode},error data:{errorData}", sendData, receivedData, DriverId);
                        }
                    }
                case McFrame.MC4E:
                    {
